...
import { FiX, FiSave, FiUser, FiUpload } from "react-icons/fi"; // Thêm FiUpload

const CustomerForm = ({ customer, onClose, onSave, customers = [] }) => {
  const { register, handleSubmit, setValue, formState: { errors } } = useForm({
    ...
    } : { gender: "Khác", status: "Hoạt động", avatar: "" }
  });

  // 🚀 STATE: Lưu trữ ảnh xem trước
  const [avatarPreview, setAvatarPreview] = useState(customer?.avatar || "");

  // Đăng ký trường avatar ngầm cho react-hook-form
  useEffect(() => {
    register("avatar");
  }, [register]);
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        // Biến ảnh thành chuỗi Base64
        const base64String = reader.result;
        setAvatarPreview(base64String); // Hiển thị xem trước
        setValue("avatar", base64String); // Lưu vào form để gửi đi
      };
  ...
  const onSubmit = (data) => {
    const submitData = {
      id: customer ? customer.id : `KH${Math.floor(Math.random() * 10000)}`,
      ...data
      // KHÔNG CÒN LẤY ẢNH TỪ RANDOM API NỮA, lấy thẳng từ data.avatar
    };
    onSave(submitData);
  };
        ...
        <div className="d-flex justify-content-between align-items-center mb-4 px-3">
          ...
          <div className="d-flex gap-2">
            ...
            <button type="submit" className="btn btn-primary text-white px-4 fw-medium shadow-sm d-flex align-items-center">
              <FiSave className="me-2" /> Lưu thông tin
            </button>
          </div>
        </div>
        ...
        <div className="card-modern bg-white p-4 mx-3 mb-4 border-0">
          ...
          <div className="d-flex flex-column align-items-center mb-4 pb-4 border-bottom">
            ...
            <label className="btn btn-outline-primary fw-medium cursor-pointer rounded-pill px-4">
              <FiUpload className="me-2" /> Chọn ảnh từ máy
              <input 
                type="file" 
                accept="image/*" 
                className="d-none" 
            ...
            <span className="text-muted small mt-2">Định dạng hỗ trợ: JPG, PNG, WEBP</span>
          </div>
          ...
          <div className="row g-4">
                ...
                {...register("phone", { 
                  ...
                  validate: {
                    isUnique: (value) => {
                      ...
                      return !isDuplicate || "Số điện thoại này đã tồn tại trong hệ thống!";
                    }
                  }
                })}
            ...
            <div className="col-md-6">
              ...
              <select {...register("gender", { required: "Vui lòng chọn" })} className="form-select bg-light border-0">
            ...
            <div className="col-12">
              ...
              <div className="d-flex gap-4 mt-1">
                <div className="form-check">
                  <input className="form-check-input" type="radio" value="Hoạt động" id="status1" {...register("status")} />
                  <label className="form-check-label text-success fw-bold" htmlFor="status1">Hoạt động</label>
                </div>
                <div className="form-check">
                  <input className="form-check-input" type="radio" value="Tạm khóa" id="status2" {...register("status")} />
                  <label className="form-check-label text-danger fw-bold" htmlFor="status2">Tạm khóa</label>
                </div>
              </div>
            </div>
          </div>
